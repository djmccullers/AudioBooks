...
function mp3Link(book, content) {
  ...
  return "<a href='mp3/" + encodeURIComponent(book.mp3).replace(" ", "+") + "'>"
      + content + "</a> ";
}
...
function authorLink(book) {
  ...
  if (book.author_url && book.author_url.substring(0, 4) == 'http') {
    return "<a href='" + book.author_url + "'>" + asString(book.author)
        + "</a> ";
  }
...
function publisherLink(book) {
  ...
  return "<a href='https://www.audible.com/search?searchProvider="
      + encodeURIComponent(book.publisher) + "'>" + asString(book.publisher)
      + "</a> ";
}
...
function narratorLink(book) {
  ...
  return "<a href='https://www.audible.com/search?searchNarrator="
      + encodeURIComponent(book.narrated_by).replace(" ", "+") + "'>"
      + asString(book.narrated_by) + "</a> ";
}

function bookImage(book, addLink) {
  var image = (book && book.image && book.image.length > 0)
      ? ("<img src='thumb/" + encodeURIComponent(book.image).replace(" ", "+")
